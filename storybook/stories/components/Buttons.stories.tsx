...
Generic.argTypes = {
  variant: {
    name: "Variant",
    options: ["primary", "secondary", "inverse", "link"] as ButtonVariant[],
    control: {
      type: "select",
      labels: {
        primary: "Primary",
      ...
      <tbody>
        {rows.map(({ $label, cellStyle: rowStyle, ...row }) => (
          <tr key={$label}>
            ...
            {cols.map(
              ({ $label: columnLabel, cellStyle: colStyle, ...col }) => (
                <TD key={columnLabel} align="left">
                  <Box css={{ ...rowStyle, ...colStyle }}>
                    <Component {...row} {...col} {...rest}>
                      {text}
                    </Component>
                  </Box>
                </TD>
