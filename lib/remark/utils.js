...
function getAttribute (node, name) {
  return node.properties?.[name] || node.data?.hProperties?.[name]
}
...
function setAttribute (node, name, value) {
  if (node.properties) {
    ...
    node.data.hProperties[name] = value
    return true
  }
}

function addClass (node, ...classes) {
  return setAttribute(node, 'className', classNames(getAttribute(node, 'className'), ...classes))
}
